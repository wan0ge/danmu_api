...
export default class RenrenSource extends BaseSource {
  ...
  async fetchStandardDanmu(url, headers, tierName) {
    try {
      ...
      const resp = await httpGet(url, { headers, timeout: 4000, sourceName: 'renren', validStatusCodes: [404] });
  ...
  async renrenHttpGet(url, { params = {}, headers = {}, validStatusCodes = [] } = {}) {
    ...
    const resp = await httpGet(u, {
      headers: headers,
      timeout: 4000, sourceName: 'renren',
      validStatusCodes
    });
    return resp;
  }
  ...
  async searchAppContent(keyword, size = 30) {
    try {
      ...
      const resp = await httpGet(url, { headers, timeout: 4000, sourceName: 'renren' });
  ...
  async performWinSearch(keyword, size = 30) {
    try {
      ...
      const resp = await httpGet(`${url}?${queryString}`, { headers, timeout: 4000, sourceName: 'renren' });
      if (!resp.data || resp.data.code !== "0000") {
